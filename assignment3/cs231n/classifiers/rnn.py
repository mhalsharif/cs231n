...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        h, cache_h = rnn_forward(x, h0, Wx, Wh, b) if (self.cell_type == 'rnn') else lstm_forward(x, h0, Wx, Wh, b)
        scores, cache_s = temporal_affine_forward(h, W_vocab, b_vocab)
        loss, dscores = temporal_softmax_loss(scores, captions_out, mask)

        #backward pass
        dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dscores, cache_s)
        dx, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, cache_h) if (self.cell_type == 'rnn') else lstm_backward(dh, cache_h)
